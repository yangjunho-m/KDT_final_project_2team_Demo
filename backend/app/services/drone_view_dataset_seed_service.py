...
def seed_drone_view_dataset(db: Session) -> dict[str, object] | None:
    ...
    db.flush()
    drone_seed_result = _upsert_dataset_drones(db, area.id, rows)
    _upsert_scenario_templates(db, area, rows)
    db.commit()
    return {
        "operationAreaId": area.id,
        "templateId": DATASET_DEMO_TEMPLATE_ID,
        **drone_seed_result,
    }
...
def _upsert_operation_area(
    ...
    *,
    ...
    settings = get_settings()
    center_latitude = settings.drone_view_operation_center_latitude
    center_longitude = settings.drone_view_operation_center_longitude
...
def _upsert_dataset_drones(db: Session, area_id: str, rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    upserted_drone_ids: list[str] = []

    for drone_id in [*preferred_ids, *fallback_ids][:3]:
        upserted_drone_ids.append(drone_id)
        drone_rows = sorted(
        ...
        record.mission_type = record.mission_type or "?뺤같"
        ...
        record.updated_at = datetime.now(UTC)
    return {
        "droneIds": upserted_drone_ids,
        "droneCount": len(upserted_drone_ids),
        "droneCRowCount": len(rows_by_drone.get("DRONE_C", [])),
    }
...
def _required_float(value: str | None, field_name: str) -> float:
    ...
    if parsed is None:
        raise ValueError(f"metadata.csv??{field_name} 醫뚰몴媛 ?놁뒿?덈떎.")
    return parsed
