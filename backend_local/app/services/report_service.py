    ...
    *,
    ...
    if client_request_id:
        existing_report = db.scalar(
            select(ReportRecord).where(ReportRecord.client_request_id == client_request_id)
        )
    ...
    for index, attachment in enumerate(attachments or [], start=1):
        record.attachments.append(
            ReportAttachmentRecord(
                id=f"ATT-{report_id.removeprefix('RPT-')}-{index:03d}",
...
def create_report_attachment_record(
    ...
    *,
    ...
    attachment = ReportAttachmentRecord(
        id=_next_attachment_id(db, report_id),
        report_id=report_id,
        file_name=file_name,
...
def _next_report_id(db: Session) -> str:
    total = db.scalar(select(func.count()).select_from(ReportRecord)) or 0
    return f"RPT-{total + 1:03d}"


def _next_attachment_id(db: Session, report_id: str) -> str:
    total = (
        db.scalar(
            select(func.count())
            .select_from(ReportAttachmentRecord)
            .where(ReportAttachmentRecord.report_id == report_id)
        )
        or 0
    )
    report_number = report_id.removeprefix("RPT-")
    return f"ATT-{report_number}-{total + 1:03d}"
